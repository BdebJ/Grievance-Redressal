...
CREATE TABLE `customerdefects` (
  `custDefId` int NOT NULL AUTO_INCREMENT,
  `prodOwnId` int NOT NULL,
  `customerDefect` varchar(200) NOT NULL,
  PRIMARY KEY (`custDefId`),
  KEY `prodOwnId_idx` (`prodOwnId`),
  CONSTRAINT `prodOwnId` FOREIGN KEY (`prodOwnId`) REFERENCES `productowned` (`prodOwnId`)
